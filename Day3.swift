...
var gridSize = (8192*2*2)+1
var start = (x: Int(gridSize/2), y: Int(gridSize/2))
print("Start: \(start)")

var grid1: [[String]] = []
var grid2: [[String]] = []
var collisions: [(x: Int, y: Int)] = []

func palceWire(with moves: [String], compGrid: [[String]]? = nil, pointToReach: (x: Int, y: Int)? = nil) -> (gird: [[String]], steps: Int) {
    var steps: [Int] = []
    var currentStep = 0
    var currentPoint = start
    var grid = Array.init(repeating: Array.init(repeating: ".", count: gridSize), count: gridSize)

    for move in moves {
        let operation = move.prefix(1)
        let distance = Int(String(move.suffix(move.count-1)))!
        ...
        case "R":
            for _ in 1...distance {
                ...
                grid[currentPoint.y][currentPoint.x] = "-"
                if let compGrid = compGrid, compGrid[currentPoint.y][currentPoint.x] != "." {
                    collisions.append((x: currentPoint.x, y: currentPoint.y))
                }
                currentStep += 1
                if (pointToReach != nil) && (pointToReach! == currentPoint) {
...
var n = 0
let cn = collisions.count
for collision in collisions {
    n += 1
    print("\(n) / \(cn)")
    steps1.append(palceWire(with: wire1Moves, pointToReach: collision).steps)
    steps2.append(palceWire(with: wire2Moves, pointToReach: collision).steps)
}

for n in 0..<steps1.count {
    commonSteps.append(steps1[n] + steps2[n])
}
...
print("Answer 2: \(commonSteps.min()!)")
